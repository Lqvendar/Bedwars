...
import me.lavender.bedwars.Bedwars;
...
public abstract class NPC extends Reflections {
    ...
    public NPC() {
        ...
        gameprofile = new GameProfile(UUID.randomUUID(), parameters.name());
    ...
    public void spawn(Player player) {
        ...
        }.runTaskLater(Bedwars.getInstance(), 80);
    }
            ...
            Bukkit.getScheduler().runTaskTimer(Bedwars.getInstance(), () -> {
                ...
                Bukkit.getOnlinePlayers().forEach(player -> {
                    ...
                    for (NPC npc : NPC.getNpcs()) {
                        List<String> viewiing = NPCRegistry.VIEWING_NPCS.get(player.getName());
                        ...
                        if (npc.getLocation().distance(player.getLocation()) >= 60) {
                            ...
                            NPCRegistry.VIEWING_NPCS.put(player.getName(), viewiing);
                            ...
                            npc.despawn(player);
                            Bukkit.getScheduler().runTaskLater(Bedwars.getInstance(), () -> npc.spawn(player), 5);
